...
- **The rail overlay** is generated by `build-tube.mjs` from Overpass API queries. OSM's own colour tags are unreliable, so TfL lines are matched by line name and National Rail by operator brand; each physical track is drawn once per line. Lines are opaque, so where several share a track the geometry is split into side-by-side bands — two lines get half the width each, four a quarter — rather than one hiding another.
